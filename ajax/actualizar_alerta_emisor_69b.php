...
try{
    ...
    if($accion==='vista'){
        $st=$pdo->prepare("UPDATE sat_69b_alertas_emisores SET estatus_seguimiento='VISTA',id_usuario_visto=?,fecha_visto=NOW() WHERE id_emisor=? AND id_empresa=? AND activa=1 AND estatus_seguimiento='NUEVA'");
        $st->execute([$uid,$id,$idEmpresa]);
    }elseif($accion==='atendida'){
        $st=$pdo->prepare("UPDATE sat_69b_alertas_emisores SET estatus_seguimiento='ATENDIDA',id_usuario_atendio=?,fecha_atendido=NOW(),observaciones=? WHERE id_emisor=? AND id_empresa=? AND activa=1");
        $st->execute([$uid,trim((string)($_POST['observaciones']??'')),$id,$idEmpresa]);
    }elseif($accion==='reabrir'){
        $st=$pdo->prepare("UPDATE sat_69b_alertas_emisores SET estatus_seguimiento='VISTA',id_usuario_atendio=NULL,fecha_atendido=NULL WHERE id_emisor=? AND id_empresa=? AND activa=1");
        $st->execute([$id,$idEmpresa]);
    }else throw new RuntimeException('Acción no válida.');
    echo json_encode(['success'=>true]);
}catch(Throwable $e){seguridad_log_error($e,'actualizar_alerta_emisor_69b');http_response_code(500);echo json_encode(['success'=>false,'error'=>$e->getMessage()],JSON_UNESCAPED_UNICODE);}
